# Задание №3:
# Массив размером 2m + 1, где m — натуральное число, заполнен случайным образом.
# Найдите в массиве медиану. Медианой называется элемент ряда, делящий его на
# две равные части: в одной находятся элементы, которые не меньше медианы, в другой — не больше медианы.
# Примечание: задачу можно решить без сортировки исходного массива. Но если это слишком сложно,
# используйте метод сортировки, который не рассматривался на уроках (сортировка слиянием также недопустима).
import random


def dwarf(arr):
    i = 1
    while i < len(arr):
        if arr[i - 1] <= arr[i]:
            i += 1
        else:
            arr[i - 1], arr[i] = arr[i], arr[i-1]
            if i > 1:
                i -= 1
    return arr


M = int(input('Введите число М для создание массива размером 2m + 1: '))
# Не очень понял, нужно ли для создания массива вставлять формулу 2m + 1, на всякий случай добавил.
arr = [random.randint(0, 99) for i in range(2 * M + 1)]
print(arr)

dwarf(arr)
print(arr)
print(f'Медиана: {arr[M]}.')
